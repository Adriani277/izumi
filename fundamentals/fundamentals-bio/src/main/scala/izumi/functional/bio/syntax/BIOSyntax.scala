...
object BIOSyntax {
  ...
  class BIOParallelOps[F[+_, +_], +E, +A](protected[this] val r: F[E, A])(implicit protected[this] val F: BIOParallel[F]) {
    ...
    @inline final def zipParLeft[E1 >: E, B](that: F[E1, B]): F[E1, A] = F.zipWithPar(r, that)((a, _) => a)
    @inline final def zipParRight[E1 >: E, B](that: F[E1, B]): F[E1, B] = F.zipWithPar(r, that)((_, b) => b)
  }

  trait BIOAsyncOps0[F[+_, +_], +E, +A] {
    protected[this] val r: F[E, A]
    implicit protected[this] val F: BIOAsync[F]

    @inline final def race[E1 >: E, A1 >: A](that: F[E1, A1]): F[E1, A1] = F.race(r, that)
  }

  class BIOAsyncOps[F[+_, +_], +E, +A](override protected[this] val r: F[E, A])(implicit override protected[this] val F: BIOAsync[F]) extends BIOOps(r) with BIOAsyncOps0[F, E, A]
  class BIOAsyncOps2[F[+_, +_], +E, +A](override protected[this] val r: F[E, A])(implicit override protected[this] val F: BIOAsync[F]) extends BIOParallelOps(r) with BIOAsyncOps0[F, E, A]

  final class BIOTemporalOps[F[+_, +_], +E, +A](override protected[this] val r: F[E, A])(implicit override protected[this] val F: BIOTemporal[F]) extends BIOAsyncOps0[F, E, A] {
  ...
  trait BIOImplicitPuns2 extends BIOImplicitPuns3 {
    ...
    @inline final def BIOAsync[F[+_, +_]: BIOParallel]: BIOParallel[F] = implicitly
  }
